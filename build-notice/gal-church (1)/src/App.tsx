...
const PAGE_META: Record<PageId, { title: string; description: string }> = {
  ...
  notice: {
    title: '공지사항 | GAL CHURCH',
    description: 'GAL CHURCH의 새로운 소식과 주요 안내를 전해드립니다.',
  },
  counseling: {
...
function MainLayout() {
  ...
  const handleNavigate = (page: PageId, sectionId?: string) => {
    ...
    if (page === 'notice') {
      ...
      if (sectionId === 'edit') {
        navigate('/notice/write?w=u&bo_table=notice&wr_id=1');
        return;
      }
      navigate('/notice');
      return;
    ...
    <div className="min-h-screen flex flex-col bg-[#F6F8FB] text-[#0F172A] font-sans selection:bg-[#198AD6]/20 selection:text-[#073063]">
      ...
      <main className="flex-1">
        <Routes>
          ...
          <Route path="/notice" element={<NoticePage onNavigate={handleNavigate} />} />
          <Route path="/notice/write" element={<NoticePage onNavigate={handleNavigate} />} />
          <Route path="/notice/:id" element={<NoticePage onNavigate={handleNavigate} />} />
          <Route path="/notice/edit/:id" element={<NoticePage onNavigate={handleNavigate} />} />
